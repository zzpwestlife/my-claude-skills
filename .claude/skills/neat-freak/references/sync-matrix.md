...
| 认证/鉴权方式变化（token / session / scope） | `docs/integration-guide.md` Auth 章节(含示例) · `docs/architecture.md` Auth/Threat Model · `docs/operator-runbook.md` 密钥轮换/排障 |
| 错误码 / 响应结构变化 | `docs/integration-guide.md` 错误码表/响应示例 ·（如有）`docs/api-reference.md` · README 快速验证示例 |
| 事件/消息 schema 变化（webhook / MQ / Kafka） | `docs/integration-guide.md` 事件 schema + 示例 payload · `docs/architecture.md` Event Flow ·（如有）consumer 侧 docs |
| 配置文件格式变化（yaml/json/toml） | README 配置步骤 · `docs/operator-runbook.md` 配置说明/默认值 ·（如有）`docs/config-reference.md` |
| 依赖升级 / runtime 版本变化（Node/Python/DB） | README prerequisites · `docs/operator-runbook.md` 升级步骤/兼容性 · 项目根指令文件（如有工具链约束） |
| 性能/限流/配额策略变化 | `docs/integration-guide.md` rate limit/重试建议 · `docs/operator-runbook.md` 指标与告警 · `docs/architecture.md` 取舍说明 |
